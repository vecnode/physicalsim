...
Desktop simulator host: a native `webview` shell with a local HTTP server
(embedded static assets) serving a browser-side simulator control UI. Each
simulator (rp2040, avr8) runs behind a common adapter interface, isolated in
its own Web Worker. The JS/TS adapters are the only place simulators
actually execute — the native shell hosts that JS runtime (via the embedded
webview) and exposes it over HTTP for external callers (e.g. droidcli) via
the native<->JS bridge below.
...
.\build\Debug\webview-app.exe --headless
...
- `public/` is generated by `npm run build` in `web/` and embedded into the executable with `cpp-embedlib`, served through the app's local HTTP server. Run the web build before configuring/building with CMake.
- The native shell (webview + httplib + cpp-embedlib) is intentionally kept as-is rather than swapped for Ultralight/CEF — cross-platform JS-engine consistency is a known tradeoff, deferred until it's an actual problem.
- Don't run `npm install`/`npx`/`tsc` directly inside `simulators/rp2040js` or `simulators/avr8js`. Both ship a `prepare`/`prepublish` script that can emit compiled `.js`/`.d.ts` files alongside their `.ts` sources; since Vite resolves the `.js`-suffixed import specifiers those `.ts` files use (NodeNext convention) against whatever literally exists on disk, stray compiled `.js` siblings silently shadow the `.ts` source for esbuild — bypassing this project's `useDefineForClassFields:false` fix (see `web/shell/vite.config.ts`) and reintroducing a real use-before-init runtime crash in several peripheral classes. If this ever happens, `git clean -fdx src` inside the affected submodule.
